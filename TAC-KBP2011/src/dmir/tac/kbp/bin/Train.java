...
public class Train {
	...
	static void extractFeatures(KBPQuery q, boolean saveToFile) throws Exception {
		...
		if (!foundCorrecEntity && !(q.gold_answer.startsWith("NIL")) ) {
			...
			TopDocs docs = dmir.tac.kbp.bin.Definitions.searcher.search(queryParser.parse(queryS), 1);				
			scoreDocs = docs.scoreDocs;
			
			if (docs.totalHits != 0) {
				Document doc = dmir.tac.kbp.bin.Definitions.searcher.doc(scoreDocs[0].doc);				
	...
	static int tune_recall(KBPQuery q) throws IOException, ParseException {
		...
		for (String sense : query.get("strings")) {
			List<SuggestWord> list = dmir.tac.kbp.bin.Definitions.spellchecker.suggestSimilar(sense, Definitions.candidates_per_sense);
		...
		for (SuggestWord suggestWord : suggestedwordsList) {
			...
			if (docs.totalHits == 0)
				...
				if (repeated.contains(docs.scoreDocs[0].doc))
					...
					Document doc = dmir.tac.kbp.bin.Definitions.searcher.doc(docs.scoreDocs[0].doc);
