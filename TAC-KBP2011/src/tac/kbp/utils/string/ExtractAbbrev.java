...
public class ExtractAbbrev {
	...
	private boolean isValidShortForm(String str) {
		return (hasLetter(str) && (Character.isLetterOrDigit(str.charAt(0)) || (str
				.charAt(0) == '(')));
	}
	...
	public Vector<Abbreviations> extractAbbrPairs(String inFile) {
					...
					if (shortForm.length() > 0 || longForm.length() > 0) {
						if (shortForm.length() > 1 && longForm.length() > 1) {
							...
							if (isValidShortForm(shortForm)) {
								extractAbbrPair(shortForm.trim(),longForm.trim(), candidates);								
							}
						}
	...
	private void extractAbbrPair(String shortForm, String longForm, Vector<Abbreviations> candidates) {
		String bestLongForm;
		...
		if (testMode) {
			...
			candidates.add(new Abbreviations(shortForm,bestLongForm));
		}
	}
	...
	private static void usage() {
		...
		System.err
				.println("       <filename> contains text from which abbreviations are extracted");
		System.err
				.println("       -testlist <file> = list of true abbreviation definition pairs");
	...
	public static void main(String[] args) {
		...
		Vector<Abbreviations> candid = extractAbbrev.extractAbbrPairs(filename);
		
		for (Abbreviations abbreviations : candid) {
			System.out.println(abbreviations.shortForm+'\t'+abbreviations.longForm);
		}
		if (extractAbbrev.testMode)
			System.out.println("TP: " + extractAbbrev.truePositives + " FP: "
					+ extractAbbrev.falsePositives + " FN: "
					+ extractAbbrev.falseNegatives + " TN: "
					+ extractAbbrev.trueNegatives);
	}
}
